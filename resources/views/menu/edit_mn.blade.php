...
<body style="background-color: #E1FFB1;">
    ...
    <div class="container mt-5">
        <h1 class="text-center mb-5">Edit Minuman</h1>
        <div class="d-grid gap-2 col-2 mx-auto">
        </div>
        <div class="card" style="background-color: #C7F2A4;">
            <div class="card-body">
                <form action="{{route('minuman.update', $minumen->id)}}" method="POST">
                    @csrf
                    @method('put')
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama') is-invalid @enderror" style="background-color: #FCFFB2" id="nama"
                            value="{{$minumen->nama}}" name="nama" placeholder="Masukkan Nama" required>
                        @error('nama')
                    ...
                    <div class="mb-3">
                        ...
                        value="{{$minumen->rasa}}" name="rasa" placeholder="Masukkan Rasa" required>
                        @error('rasa')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        value="{{$minumen->harga}}" name="harga" placeholder="Masukkan Harga Rp." required>
                        @error('harga')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
